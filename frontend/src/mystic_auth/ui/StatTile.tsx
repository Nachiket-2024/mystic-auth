...
// number plus a muted label), and a solid fill washed both out. brand.selected
// is a light tinted wash that sits behind existing text colors instead of
// replacing them.
const STAT_TILE_HOVER_PROPS = {
    ...
    // Plain Box as="button" gets no recipe styling (unlike a real Chakra
    // Button), so without this a keyboard user only saw the bare
    // browser-default outline, inconsistent with every other focusable
    // control's brand-colored ring. _focusVisible still works here since
    // it's a core style prop, not recipe-specific.
...
const StatTile: React.FC<StatTileProps> = ({ label, value, isLoading, color = "fg.default", onClick, ariaLabel }) => {
    ...
    >
        {isLoading ? (
            ...
            // resolves to the same gray.800 as bg.surface, making the
            // skeleton invisible even mid-pulse.
