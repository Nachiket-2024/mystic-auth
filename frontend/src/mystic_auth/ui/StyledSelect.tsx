    ...
    return (
        ...
        >
            ...
            {/* Chakra's Select.Trigger always wires aria-labelledby to
                ...
                since a labelledby pointing at a nonexistent id doesn't fall
                back to aria-label. */}
            ...
            <Select.Control>
                <Select.Trigger
                    ...
                    // (same 14px textStyle); overridden to match the 15px
                    // used for table text/buttons elsewhere.
                    fontSize="md"
            ...
            <Portal>
                <Select.Positioner>
                    {/* No explicit zIndex: Chakra's Select recipe already
                        computes a dynamic --layer-index, so this stacks above
                        any Dialog it opens from. A hardcoded zIndex used to
                        clobber that and render the dropdown behind its dialog. */}
                    <Select.Content
                        borderWidth="1px"
                        borderColor="border.default"
                        bg="bg.surface"
                        // Same density.card elevation Card.tsx uses, so cards
                        // and popovers share one look.
                        boxShadow="density.card"
                        rounded="density.control"
                        fontSize="md"
                        // Cap by row count (~7.5 rows), not a flat rem/vh guess.
                        // 36px/row = Chakra's md Select.Item py (12px) + md
                        // line-height (24px).
                        maxH="calc(7.5 * 36px)"
                        overflowY="auto"
                    >
                        {options.map((option) => (
                            <Select.Item
                                ...
                                // brand fill on hover, tinted wash + checkmark
                                // for the selection.
                                _highlighted={{ bg: "brand.solid", color: "white" }}
                                // Nested _highlighted here too, or hovering the
                                // selected row renders invisible same-color
                                // text (brand.fg == brand.solid in light).
                                _selected={{
                                    bg: "brand.200",
                                    color: "brand.fg",
                                    fontWeight: "semibold",
