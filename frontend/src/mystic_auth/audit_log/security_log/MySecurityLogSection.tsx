...
import { useMySecurityLogUiStore } from "./securityLogUiStore";
...
const MySecurityLogSection: React.FC = () => {
    ...
    // Backed by useMySecurityLogUiStore - see AllSecurityLogSection's matching comment.
    const { eventType, ipAddress, success, sortKey, sortDir, range, customFrom, customTo, update } =
        useMySecurityLogUiStore();
    ...
    const [page, setPage] = usePageResetOn(
        `${sort.key}|${sort.direction}|${eventType}|${debouncedIpAddress}|${success}|${rangeParams.from}|${rangeParams.to}`
    );

    const { data, error, isFetching, isLoading, isError, isPlaceholderData } = useMySecurityAuditLogQuery(page, PAGE_SIZE, {
    ...
    const trend = useMyLoginTrendQuery(rangeParams.days, rangeParams.from, rangeParams.to);
    ...
    const hasNonRangeFilters = eventType !== ALL_VALUE || !!ipAddress || success !== ALL_VALUE;
    const isRangeActive = range !== DEFAULT_TIME_RANGE || (range === "custom" && !!customFrom && !!customTo);
    const clearAll = () =>
        update({ eventType: ALL_VALUE, ipAddress: "", success: ALL_VALUE, range: DEFAULT_TIME_RANGE, customFrom: "", customTo: "" });
    ...
    const emptyMessage = showRangeEmpty ? t("security.emptyRange", { days: rangeParams.days }) : t("security.emptyFiltered");
