...
import { useMyAuthorizationLogUiStore } from "./authorizationLogUiStore";
 ...
 * PBAC decisions only. Search stays visible but disabled (see AuthorizationFilterBar's
 * `mine` prop) rather than hidden, since a table-wide affordance vanishing between
 * sub-tabs reads as broken. */
const MyAuthorizationLogSection: React.FC<MyAuthorizationLogSectionProps> = ({
    ...
    // Backed by useMyAuthorizationLogUiStore - see AllSecurityLogSection's matching comment.
    const { action, resourceType, allowed, sortKey, sortDir, range, customFrom, customTo, update } =
        useMyAuthorizationLogUiStore();
    ...
    const [page, setPage] = usePageResetOn(
        `${sort.key}|${sort.direction}|${action}|${resourceType}|${allowed}|${rangeParams.from}|${rangeParams.to}`
    );

    const { data, isFetching, isLoading, isError, isPlaceholderData } = useMyAuthorizationAuditLogQuery(page, PAGE_SIZE, {
    ...
    const hasNonRangeFilters = action !== ALL_VALUE || resourceType !== ALL_VALUE || allowed !== ALL_VALUE;
    const isRangeActive = range !== DEFAULT_TIME_RANGE || (range === "custom" && !!customFrom && !!customTo);
    const clearAll = () =>
        update({ action: ALL_VALUE, resourceType: ALL_VALUE, allowed: ALL_VALUE, range: DEFAULT_TIME_RANGE, customFrom: "", customTo: "" });
    ...
    return (
        <>
            ...
            {!isLoading && !isError && (data?.rows.length ?? 0) > 0 && (
                <Pagination page={page} totalPages={totalPagesFor(data?.total ?? 0)} onPageChange={setPage} className="mt-4" />
            )}
