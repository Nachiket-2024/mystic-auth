 ...
 * has to happen at the system-build level, not as a post-hoc CSS variable
 * override - an earlier version did the latter and silently failed to apply
 * dark-mode values). Subscribing to useAppearanceStore here means every
 * place that reads these tokens reacts instantly, so AppearanceCard.tsx's
 * color pickers need no separate "preview" plumbing.
 *
 * useMemo skips rebuilding on unrelated re-renders; buildAppearanceThemeOverrides
 * returns null (reuse the shared default `system`) when nothing is customized.
 */
