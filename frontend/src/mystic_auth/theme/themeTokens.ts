...
export const tokens = {
    fonts: {
        ...
        // fallback chain is Chakra's own default, so a slow first paint
        // still gets a native font instead of serif/monospace.
    ...
    colors: {
        // Always superseded at runtime by system.ts's `brandDefault`, generated
        // from settings.ts's BRAND_COLOR; this teal scale is just the baseline
        // that generator merges over.
        brand: {
            50: { value: "#e6fffa" },
            100: { value: "#b2f5ea" },
            200: { value: "#81e6d9" },
        ...
        // highlights that should read as "a second color," not a paler
        // copy of brand. Its own token, like brand.*, so a fork can
        // override it from app/theme.ts.
        accent: {
            50: { value: "#f5f3ff" },
            100: { value: "#ede9fe" },
            200: { value: "#ddd6fe" },
    ...
    durations: {
        ...
        // (strength-meter fill, route fade-in, drawer slide), matching what
        // those call sites already hardcoded. RouteProgressBar's 1.1s loop
        // is left out: it's a continuous loading loop, not a UI response
        // speed, so it doesn't belong on this "how snappy" dial.
        fast: { value: "0.15s" },
        base: { value: "0.2s" },
    },
    easings: {
    ...
    // raw scale step, so a fork can retune "how rounded/dense" from
    // app/theme.ts without hunting down every prop. Only wired into Card,
    // PageContainer, buttons, and inputs so far; the rest still uses
    // Chakra's raw scale and can migrate over incrementally.
    radii: {
    ...
    // top-edge sheen) reads as more deliberately designed, Linear/Vercel-
    // style. Shared by Card.tsx and StyledSelect's popover. Dark mode's
    // alphas are tuned separately since the light-mode sheen disappears
