...
Request
   |
Authentication (auth/current_user)
   |
Authorization Context Builder (authorization/context/request_context_builder.py)
   |
Authorization Service (authorization/services/authorization_service.py)
   |
Policy Evaluation Engine (authorization/evaluators/policy_evaluator.py)
   |
Condition Evaluation Service (authorization/conditions/condition_evaluation_service.py)
   |
Authorization Decision (authorization/evaluators/authorization_decision.py)
   |
Audit Log (authorization/repositories/audit_log_repository.py)
```
