...
| Unit | `tests/frontend/mystic_auth/unit/` (~35 files) | API clients (`auth`/`users`/`account_settings`/`policies`/`audit` endpoints, `apiError`, the refresh interceptor), `useAuthSession`, `useSessionEventsStream` (SSE connect/disconnect/query-invalidation on push), `Authorized`/`ProtectedRoute`/`Sidebar`/`Navbar`/`AppLayout`, `useAuthorization`/`useCan`/`authorizationService` (PBAC layer), `passwordRules`/`PasswordRulesChecklist`, `useUnsavedChangesWarning`, `themeStore`, `ui/*` (`DataTable`, `ConfirmDialog`, `FormAlert`, `LoadingState`, `Toaster`, `ErrorBoundary`: including that it reports the caught error), `errorMonitoring` (conditional on `VITE_SENTRY_DSN`) |
