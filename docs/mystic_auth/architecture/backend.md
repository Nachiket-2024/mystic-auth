...
flowchart TD
    Request(["Request"])
    Correlation["CorrelationIdMiddleware<br/> outermost, sets request.state.request_id first"]
    Security["SecurityHeadersMiddleware<br/> attaches response headers on the way out"]
    Logging["LoggingMiddleware<br/> logs incoming request / outgoing response"]
    Cors["CORSMiddleware<br/> allowed origins: FRONTEND_BASE_URL +<br/> FRONTEND_ADDITIONAL_BASE_URLS"]
    Deps["Route dependency chain<br/> get_current_user / require_authorization,<br/> rate limiting"]
    Handler["Route handler"]
    Error["Global exception handler<br/> @app.exception_handler(Exception): generic 500"]
