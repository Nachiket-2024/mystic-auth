...
| `env/` | Mode-specific env examples and git-ignored real env files. | [Environment Configuration](../environment/README.md), [Deployment Guide](../deployment/environment.md#1-choosing-the-right-env-template) | Backend runtime env, frontend build-time env, Compose-only env, tunnel-specific env, required replacements, and fallback rules. |
