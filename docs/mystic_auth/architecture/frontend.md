...
| `permissions/` | `PermissionsPage.tsx`: read-only, browsable view of the fixed, code-defined permission catalog (`GET /authorization/permissions/catalog`, see `authorization/permissions_catalog.py`). Kept separate from `policies/` rather than a tab on it: this is the closed action vocabulary developers define, Policies is how admins compose it, so there's no create/edit/delete here - a permission only means something once a backend route checks for it. Search/filter/sort/paginate all run client-side against the single, small, fixed catalog rather than round-tripping per keystroke like a real list endpoint would |
| `rate_limits/` | `RateLimitsPage.tsx`: admin view of [`RateLimitDashboardService.list_active_limits`](../security/hardening-abuse-prevention.md#rate-limit-dashboard-get-rate-limits) (`GET /rate-limits`), filterable by `scope` (`ip`/`account`/`email`) via `RateLimitsFilterBar.tsx`, plus `useResetRateLimitMutation` (`rateLimitsMutations.ts`) for `DELETE /rate-limits/{key:path}` |
...
| `api/` | Axios-based typed call functions per backend domain (`auth_api`, `users_api`, `account_settings_api`, `policies_api`, `permissions_api`, `bulkAssignment_api`, `rate_limits_api`, `audit_api`), plus `axiosInstance.ts` and `apiError.ts` |
...
| `theme/` | `system.ts`: Chakra UI v3 design tokens, seeded from `BRAND_COLOR`/`VITE_BRAND_COLOR` via `generateBrandScale.ts`. `AppearanceThemeProvider.tsx`/`appearanceThemeOverrides.ts` rebuild the whole Chakra system live when a signed-in user picks their own brand color; `applyFaviconAndMetaColor.ts`/`brandIcon.ts` update the favicon and `theme-color` meta tag to match, and `themeTokens.ts`/`themeSemanticTokens.ts`/`themeStyles.ts` hold the rest of the shared token/style definitions. See [Appearance](../appearance/overview.md) for the full mechanism |
