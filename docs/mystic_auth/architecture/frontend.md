...
| `store/` | Zustand: `authStore.ts` (session/profile/permissions), `themeStore.ts` (light/dark), `languageStore.ts` (translation language mode, see [Translations Overview](../translations/overview.md)), `fontSizeStore.ts` (small/medium/large, persisted to `localStorage`, applied as a percentage of the browser's own root font size so it composes with the user's OS/browser text-size setting), `routeLoadingStore.ts` (in-flight lazy-route-chunk counter, drives `ui/RouteProgressBar.tsx`); client state only, no Redux |
...
| `layout/` | App shell: `AppLayout`, `Navbar`, `Sidebar`, `ThemeToggle`, `LanguageToggle`, `FontSizeControl`, `navItems.ts`. `AppLayout` takes an optional `extraNavItems: NavItem[]` prop so your own feature routes can add sidebar links without editing `navItems.ts` (upstream-owned), an optional `extraNavbarContent: React.ReactNode` prop so you can add your own top-bar content (buttons, icons, ...) without editing `Navbar.tsx`, and an optional `onOpenCommandPalette: () => void` prop that renders the navbar's search-trigger button for the command palette below. See [Using This Repository as a Template: shared-chrome extension points](../template-usage/overview.md#shared-chrome-extension-points). Also `CommandPalette.tsx` (Cmd+K/Ctrl+K quick-jump: gated nav destinations, in-page content, and, if permitted, user accounts; result-building logic in `CommandPaletteResults.ts`, row rendering in `CommandPaletteResultsList.tsx`), mounted once at the app root in `App.tsx` alongside the global keydown listener that toggles it |
| `ui/` | Generic reusable UI kit, no feature ownership: `DataTable` (sortable columns, loading/error/empty states), `Pagination`, `StyledSelect`, `ConfirmDialog`, `FormAlert`, `PageContainer`, `Card`, `TableActionButton`, `LoadingState`, `ErrorBoundary`, `dateFormat.ts` (fixed-locale date/time formatting, shared by `dashboard/` and its `manage_sessions/` subfolder), `trackedLazy.ts`/`RouteProgressBar.tsx`/`RouteSkeleton.tsx`/`RouteFadeIn.tsx` (route-loading UX, see [Route-loading UX](#route-loading-ux) below), `DataTableSkeleton.tsx` (list-page loading state). Single-file components live flat at the top level. Multi-file groups get subfolders: `ui/toaster/` (`toaster.tsx` + `toasterInstance.ts`), `ui/styles/` (`buttonStyles.ts`, `inputStyles.ts`, `dialogStyles.ts`), and `ui/hooks/` (`useSortState`, `useDebouncedValue`, `usePageResetOn`). See [List pages](#list-pages-pagination-sorting-filtering) below |
...
Every lazy-split route in `App.tsx` uses `ui/trackedLazy.ts`'s `trackedLazy()` instead of plain `React.lazy()`. It wraps the same dynamic `import()` but calls `startRouteLoad()`/`finishRouteLoad()` (`store/routeLoadingStore.ts`) around it, incrementing/decrementing a `pendingCount` rather than tracking a single boolean, so two overlapping navigations (e.g. a user clicking a second link before the first chunk finishes) don't clear the loading state early. Two things react to that count:

- `ui/RouteProgressBar.tsx`: a thin fixed-top indeterminate progress bar, mounted once at the app root, visible whenever `pendingCount > 0`.
...
`ui/RouteFadeIn.tsx` fades each route's content in on every navigation (mounted alongside the two above in `App.tsx`), so a route transition never pops in abruptly even on a fast, cached load.
...
Text size is a separate, independent control: `store/fontSizeStore.ts` + `layout/FontSizeControl.tsx` (a navbar dropdown, small/medium/large) scale `document.documentElement.style.fontSize` as a percentage (87.5% / 100% / 112.5%), so it multiplies with whatever base size the user's own browser/OS already has set for accessibility, rather than overriding it outright. The choice persists to `localStorage` and is applied at module load, before the app renders, to avoid a flash of default-size text.
