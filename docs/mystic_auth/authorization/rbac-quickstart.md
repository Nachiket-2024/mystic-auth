...
[Common Patterns](common-patterns.md) covers modeling *hierarchies* on top of PBAC. This page is for the opposite, simpler need: your access model really is just "everyone with role X gets exactly these actions, no per-resource scoping" — plain RBAC — and PBAC's full generality (conditions, `resource_attributes`, time/network/date-range checks) is more machinery than you need for it.
...
2. **Assign it** to whichever users should hold that role — `POST /authorization/users/{email}/policies` (or the UI). A user can hold more than one role-policy at once; PBAC evaluates every held policy, so someone with both `role_editor` and `role_viewer` simply has the union of both action lists.
