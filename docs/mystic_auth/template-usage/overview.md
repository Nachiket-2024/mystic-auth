...
If a future release adds an extension point to another shared component (e.g. the top bar), it'll follow this same shape — a typed, optional, additive prop — and get listed in this table.
...
Pulling in fixes/features from the original template once your own project has diverged from it — including the full step-by-step walkthrough and a worked conflict-resolution example — now lives in its own doc: see [Staying in Sync with Upstream Template Updates](syncing-upstream.md).
