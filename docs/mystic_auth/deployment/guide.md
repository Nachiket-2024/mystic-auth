...
- `FRONTEND_BASE_URL` / `BACKEND_BASE_URL` — must point at the real production hostnames; CORS (`main.py`) allows this origin plus any comma-separated `FRONTEND_ADDITIONAL_BASE_URLS` (e.g. a second domain, or staging sharing this backend) — leave that unset for the common single-origin case.
