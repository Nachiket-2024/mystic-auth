...
[Local-Prod](local-prod.md) when traffic should reach the server directly
...
`.env.prod.example` is the source of truth for prod values. It is set up for
...
These are the same across all three Compose files. See
