...
- The same fast, snappy hover transition (`FAST_HOVER_TRANSITION`, now owned by the shared layout/button styling) applied consistently across buttons, pagination, and table action buttons.
...
- Self-service account deletion, plus admin purge, via a new `user_lifecycle/` backend module.
- A PBAC grant guard stopping a user from granting a policy more privileged than their own.
- A command palette (Cmd+K).
- Route-loading/font-size UI infrastructure.
- hi/gu/mr translations added across every namespace.
- Fixes: an `X-Forwarded-For` spoofing bug, a command-palette i18n leak, a stale mypy suppression.
- Docs brought up to date to match, split one page per authentication flow with a diagram each.
...
- Taskiq replaced with Procrastinate for background jobs.
- Manage Sessions gained offline GeoIP-based location.
- JWT issuer/audience claims enforced.
- An admin rate-limits dashboard added, via a new `rate_limits/` frontend module.
- A user-export row cap added.
- Docs overhauled to match: stale references removed, diagrams redrawn vertically and fixed for legibility, every page restructured tutorial-style.
...
- Brand colors and legal consent pages added, via a new `landing_page/` module with its own translations.
- A least-privilege Postgres role added.
- redis session revocation fixed to properly confirm password resets, deletions, and admin actions.
- PBAC permission filtering fixed, with 5 seeded policies and pagination.
- Cookie, React, and local DB issues patched, with more tests.
- OAuth2 failure handling fixed with specific error codes for deleted, deactivated, cancelled, and expired states; rate-limited routes now redirect instead of returning raw JSON.
- Audit logging moved to the Procrastinate background worker.
- An nginx route leak fixed, and three Docker Compose databases split.
...
- Fixed a duplicate security-header bug in `nginx.frontend.conf`.
- Fixed a `DATABASE_URL` override that silently shadowed `env_file` and had broken the stack mid-audit.
- Fixed a hardcoded Docker subnet collision between the two production-style Compose files.
- Fixed a pydantic circular-reference flake in policy list serialization, by building schemas row by row.
- Rebuilt the PBAC frontend surface: bulk actions on the Users page, the permission catalog, and an effective-permissions viewer, verified across themes, font sizes, and languages.
- Attacked the backend directly the way a real attacker would (forged JWTs, IDOR, mass assignment, privilege self-granting, IP spoofing, and more), all correctly blocked.
- Added client-side length validation matching the backend's limits.
- Added a `Retry-After` header with a translated wait-time message on login lockout.
- Closed the "backups are scripted but nothing schedules them" gap with an on-by-default `db_backup` service on both production Compose files.
- Swept the docs for staleness, including the PBAC component table and color-coded flow diagrams; trimmed comments across all files and tests to be concise. See the [security decisions](../../security/decisions.md) and [concerns](../../concerns/README.md) pages for the lasting details.
