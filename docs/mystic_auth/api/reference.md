...
| GET | `/users/export` | `users:list_all` | CSV export of every user matching `search`/`role`/`is_verified`/`status` (no `limit`/`offset`: always the whole filtered set). Rejected with `400 EXPORT_TOO_LARGE` if the filtered set exceeds `USER_EXPORT_MAX_ROWS` (`env/mystic_auth/.env.example`, default `50000`); narrow the filters instead |
