...
| Content | `frontend/src/mystic_auth/translations/languages/<lang>/legal.json` | All document text, per language. Registered as its own `legal` i18next namespace at runtime (`frontend/src/app/translations/registerLegalTranslations.ts`), alongside the other app-owned locale registrations. It stays outside `mystic_auth/translations/translations.ts`'s upstream `NAMESPACES` list. |
