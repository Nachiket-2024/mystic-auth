...
A Valkey outage means `bump_account_version`/`bump_chain_version` cannot be confirmed, so nothing was actually revoked. Each caller decides what that means for its own response, rather than one blanket policy:
...
| `PUT /users/me` password change | The password write itself (Postgres, independent of Valkey) still succeeds: blocking a password change on an unrelated Valkey outage would be worse than the gap it's protecting against. The response carries `sessions_revoked: false` instead, so the caller (and `ChangePasswordCard.tsx`, which shows a distinct warning toast) can see that the account's *other* sessions were not actually signed out. |
...
| `DELETE /users/me` self-delete, and `POST /users/me/confirm-delete` (OAuth-only) | Both go through `finalize_self_deletion`, which soft-deletes first (a Postgres write, unrelated to Valkey) and always succeeds regardless of the bump outcome. An unconfirmed bump is logged at `critical` and recorded as `sessions_revoked_confirmed: false` in the `ACCOUNT_DELETED_SELF` audit metadata, but the HTTP response is unaffected - the account really is gone either way. |
