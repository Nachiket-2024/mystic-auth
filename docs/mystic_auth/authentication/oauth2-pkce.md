...
- **System account is blocked from OAuth2 login entirely** — `role == UserRole.system` short-circuits before any user creation/update logic, forcing the reserved system account through password login only (`scripts/create_system_user.py`). See [System Superuser: Bootstrapping and Promotion](system-superuser.md) for why this script won't promote a Google-only account (no password at all) in place — it offers to delete and recreate it instead — and why promoting any other existing account sets its `role` to `system` as part of the promotion, deliberately cutting off its own Google login going forward.
