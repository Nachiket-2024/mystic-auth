...
REM (docker/compose/docker-compose.prod.yml). Fill in
REM local-scripts\prod\system-user.env first, with a real production
...
REM `docker compose -f docker/compose/docker-compose.prod.yml --env-file env/.env.prod exec backend python -m mystic_auth.scripts.create_system_user`
