...
# (docker/mystic_auth/compose/docker-compose.prod.yml). Fill in
...
try {
    ...
    cmd /c "docker compose -f docker/mystic_auth/compose/docker-compose.prod.yml -f docker/app/compose/docker-compose.prod.yml --env-file env/mystic_auth/.env.prod --env-file env/app/.env.prod exec -T backend python -m mystic_auth.scripts.create_system_user < ""$tempFile"""
} finally {
