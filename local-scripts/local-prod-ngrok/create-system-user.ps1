...
# stack (docker/compose/docker-compose.local-prod-ngrok.yml). Fill in
...
# `docker compose -f docker/compose/docker-compose.local-prod-ngrok.yml --env-file env/.env.local-prod-ngrok exec backend python -m mystic_auth.scripts.create_system_user`
...
try {
    ...
    cmd /c "docker compose -f docker/compose/docker-compose.local-prod-ngrok.yml --env-file env/.env.local-prod-ngrok exec -T backend python -m mystic_auth.scripts.create_system_user < ""$tempFile"""
} finally {
