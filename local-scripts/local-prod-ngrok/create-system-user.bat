...
REM stack (docker/compose/docker-compose.local-prod-ngrok.yml). Fill in
REM local-scripts\local-prod-ngrok\system-user.env first. Assumes a fresh account
...
REM `docker compose -f docker/compose/docker-compose.local-prod-ngrok.yml --env-file env/.env.local-prod-ngrok exec backend python -m mystic_auth.scripts.create_system_user`
...
) | docker compose -f docker/compose/docker-compose.local-prod-ngrok.yml --env-file env/.env.local-prod-ngrok exec -T backend python -m mystic_auth.scripts.create_system_user
