...
async def test_login_or_create_user_existing_user_does_not_write_orphaned_session_store(mocker):
    ...
    # The old `user_tokens:{email}` list was dead code nothing ever read
    # and grew forever with no TTL; it must no longer be written to.
...
async def test_login_or_create_user_clears_password_on_pre_hijacked_unverified_account(mocker):
    # Pre-hijacking regression guard: an attacker can register the
    # victim's email with an attacker-chosen password and never verify
    # it. If linking only flipped is_verified without clearing
    # hashed_password, the attacker's password would remain valid on the
    # now-verified account, letting them log in as the victim after the
    # victim's first "Sign in with Google". hashed_password must be
    # cleared so the attacker-set credential can't survive the claim.
    mocker.patch(
...
async def test_login_or_create_user_rejects_reserved_system_account(mocker):
    # OAuth2 login trusts Google's email_verified alone; there's no
    # password check in this flow. Without this guard, anyone who
    # controls a Google account matching the reserved system superuser's
    # email could sign in as it, bypassing its password entirely. Mirrors
    # the same role == UserRole.system guard in
    # user_management_update_routes.py / user_lifecycle_routes.py.
    ...
    # Generic code, not a distinct one: the frontend/user must not be
    # able to tell "this is the reserved system account" apart from any
    # other OAuth2 login failure.
...
async def test_login_or_create_user_creates_new_verified_user(mocker):
    ...
    # same default UserRole.user password signup uses, so every new
    # account shows a role consistently in the UI regardless of how it
    # was created.
...
async def test_login_or_create_user_normalizes_google_email_casing_for_lookup(mocker):
    ...
    # Pydantic schema, so oauth2_service.py must normalize it itself
    # before calling get_by_email, unlike the signup/login paths.
