...
async def test_logout_all_with_undecodable_token_still_succeeds_and_clears_cookies(mocker):
    ...
    # just garbage) carries no recoverable email, so there's nothing left
...
async def test_logout_all_with_wrong_type_token_does_not_revoke_but_still_clears_cookies(mocker):
    ...
    # an access token presented as the refresh_token cookie) must never be
    # treated as resolving a real email to revoke sessions for, same as
    # refresh_tokens() in refresh_token_service.py. It still clears
    # cookies and reports success, consistent with every other
    # invalid-token case here.
    mocker.patch(
...
async def test_logout_all_with_already_revoked_token_still_resolves_email_and_revokes_remaining_sessions(mocker):
    # Regression guard: right after a password change (which revokes
    # every refresh token for the account), this device's own
    # refresh-token cookie is already revoked. jwt_service.verify_token
    # would reject it outright (no email recoverable), but decode_payload
    # (which skips the revocation check, same as reuse-detection in
    # refresh_token_service) still yields the owning email, so logout-all
    # can still revoke whatever sessions remain and clear cookies instead
    # of erroring out.
    mocker.patch(
        f"{MODULE}.jwt_service.decode_payload",
        new_callable=AsyncMock,
...
async def test_logout_all_with_undecodable_token_records_audit_entry_with_no_email(mocker):
    ...
    # up) rather than skipping the audit call.
    mocker.patch(f"{MODULE}.jwt_service.decode_payload", new_callable=AsyncMock, return_value=None)
    audit_mock = mocker.patch(f"{MODULE}.log_security_event", new_callable=AsyncMock)
