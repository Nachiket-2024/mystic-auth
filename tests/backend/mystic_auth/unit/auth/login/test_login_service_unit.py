# Regression guard for a login timing side-channel: login() previously
# returned immediately (skipping the Argon2 comparison) for "user not
# found" and "user unverified", but did a real hash comparison for "wrong
# password on an existing, verified account". An attacker could
# distinguish these cases by response latency alone, enabling account
# enumeration despite every branch returning the same generic failure.
# login() now always performs exactly one comparison, against the user's
# real hash if one exists or a fixed dummy hash otherwise, before any
# not-found/unverified branching.
...
MODULE = "backend.mystic_auth.auth.login.login_service"
...
async def test_login_nonexistent_user_still_performs_a_hash_comparison(mocker):
    ...
    # Must compare against the fixed dummy hash, never skip the
    # comparison, so this branch's timing matches a genuine
...
async def test_login_deactivated_account_is_blocked_even_with_correct_password(mocker):
    # A deactivated account must not receive tokens, even with a correct
    # password: current_user_handler.py would reject the tokens on first
    # use anyway, but issuing them here is wasteful and misleading.
...
async def test_dummy_hash_is_a_real_argon2_hash_not_a_placeholder_string():
    ...
    # real Argon2 work on it. A plain sentinel string would let the verify
    # call short-circuit before hashing, reopening the timing gap this
    # fix closes.
