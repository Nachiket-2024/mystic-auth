# tests/backend/mystic_auth/unit/taskiq_tasks/test_account_purge_tasks_unit.py
#
# Mocked-collaborator coverage for the scheduled purge task's own wiring
# (label/schedule shape, which CRUD/service calls it makes and with what
...
MODULE = "backend.mystic_auth.taskiq_tasks.account_purge_tasks"


def test_purge_task_is_labeled_with_a_daily_cron_schedule():
    schedule = purge_expired_soft_deleted_accounts.labels.get("schedule")
...
async def test_purge_task_purges_every_account_returned_by_get_deleted_before(mocker):
    ...
    result = await purge_expired_soft_deleted_accounts()
...
def test_scheduler_includes_a_label_schedule_source():
    from taskiq.schedule_sources import LabelScheduleSource

    from backend.mystic_auth.taskiq_tasks.email_tasks import scheduler
